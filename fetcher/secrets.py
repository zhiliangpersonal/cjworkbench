...
import aiohttp
from cjwkernel.types import I18nMessage, make_i18nMessage
...
def _service_no_longer_configured_error(service: str):
    return make_i18nMessage(
        ...
        "Service {service} is no longer configured",
        {"service": service},
    )
    ...
    try:
            ...
            async with session.post(
                ...
                if (response.status // 400) == 1 and "error" in body:
                    ...
                    raise _RefreshOauth2TokenError(
                        make_i18nMessage(
                            "py.fetcher.secrets._refresh_oauth2_token.oauth_error",
                            "Token server responded with {status_number}: {error} ({description})",
                            {
                ...
                if response.status != 200:
                    ...
                    raise _RefreshOauth2TokenError(
                        make_i18nMessage(
                            "py.fetcher.secrets._refresh_oauth2_token.server_error",
                            "{service_id} responded with HTTP {status_number} {error}: {description}",
                            {
        ...
        raise _RefreshOauth2TokenError(
            make_i18nMessage(
                ...
                "Timeout during OAuth2 token refresh",
            )
        )
        ...
        raise _RefreshOauth2TokenError(
            make_i18nMessage(
                ...
                "HTTP error during OAuth2 token refresh: {error}",
                {"error": str(err)},
            )
        )
